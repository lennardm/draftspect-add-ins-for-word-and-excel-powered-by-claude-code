...
> **Do I need to install "Claude Code" separately?** **No.** Draftspect ships
> the Claude Code engine inside its dependencies — `npm install` (Step 4)
> downloads it automatically. And because we're using OpenRouter, there is **no
> Claude login or OAuth** — the OpenRouter API key (Step 5) is all the
...
3. *(skipped — no separate Claude Code install needed)*
4. Run `npm install` inside the Draftspect folder
5. Paste your OpenRouter key into a `.env` file, then start it
...
There is **no separate Claude Code installation**. The next step pulls it in
automatically. Move on.

---

## Step 4 — Install Draftspect's components
...
## Step 5 — Add your OpenRouter key and start it
...
## Step 6 — Install the add-in into Word and Excel
